...
class WebScraper:
    ...
    def DataDisplay(self): 
        ...
        if (choice == 'B'):
            ...
            categories = ["Bid/Ask", "1 Year Price Target", "52 Week High/Low", "Market Cap", "P/E", "EPS", "Beta",]
            ...
            for sym in symbols:
                if (nameCol):
                    print("Statistic", end="")
                    self.AlignTable("Statistic")
                print(sym.upper(), end="") 
                self.AlignTable(sym)
                nameCol = False
            self.AlignTable("", True, len(symbols)+1)

            for i in range(0, len(bidAskList) // len(symbols)):
                for j in range(0, len(symbols)+1):
                    if (j == 0): 
                        print(categories[0], end="")
                        self.AlignTable(categories[0])
                    else:
                        print(bidAskList[i+j-1], end ="")
                        self.AlignTable(bidAskList[i+j-1])
                print("")                
                for j in range(0, len(symbols)+1):
                    if (j == 0): 
                        print(categories[1], end="")
                        self.AlignTable(categories[1])
                    else:
                        print(targetList[i+j-1], end ="")
                        self.AlignTable(targetList[i+j-1])
                print("")
                for j in range(0, len(symbols)+1):
                    if (j == 0): 
                        print(categories[2], end="")
                        self.AlignTable(categories[2])
                    else:
                        print(highLowList[i+j-1], end ="")
                        self.AlignTable(highLowList[i+j-1])
                print("")
                for j in range(0, len(symbols)+1):
                    if (j == 0): 
                        print(categories[3], end="")
                        self.AlignTable(categories[3])
                    else:
                        print(capList[i+j-1], end ='')
                        self.AlignTable(capList[i+j-1])
                print("")
                for j in range(0, len(symbols)+1):
                    if (j == 0): 
                        print(categories[4], end="")
                        self.AlignTable(categories[4])
                    else:
                        print(peList[i+j-1], end ="")
                        self.AlignTable(peList[i+j-1])
                print("")
                for j in range(0, len(symbols)+1):
                    if (j == 0): 
                        print(categories[5], end="")
                        self.AlignTable(categories[5])
                    else:
                        print(epsList[i+j-1], end ="")
                        self.AlignTable(epsList[i+j-1])
                print("")
                for j in range(0, len(symbols)+1):
                    if (j == 0): 
                        print(categories[6], end="")
                        self.AlignTable(categories[6])
                    else:
                        print(betaList[i+j-1], end ="")
                        self.AlignTable(betaList[i+j-1])
                print("")
            print("")
